...
import com.example.Baloot_Shopping_Center_Phase3.User_Comment;

import java.io.IOException;
import java.util.List;

public class Commodity_Page_Handler {
    public String Response(int product_id, String current_logged_in_username, boolean add_to_cart, boolean remove_from_cart, String comment, int comment_id, int comment_rate) throws IOException {
        if(add_to_cart)
            Shopping_WebSite.get_Shopping_WebSite_instance().add_to_buy_list(current_logged_in_username, product_id);
        if(remove_from_cart)
            Shopping_WebSite.get_Shopping_WebSite_instance().remove_from_buy_list(current_logged_in_username, product_id);
        if(!comment.isEmpty())
                ...
                "<!Doctype html>" +
                        ...
                        "<html>" +
                            ...
                            "<body>" +
                                ...
                                "<script type=\"text/babel\">" +
                                    "ReactDOM.render(<ProductInfo product_id=\"" + product_id + "\" product_provider=\"" + product_provider_name + "\" username=\"" + current_logged_in_username + "\" number_of_this_commodity_in_cart=\"" + number_of_this_commodity_in_cart + "\"/>, document.getElementById(\'product-info\'));" +
